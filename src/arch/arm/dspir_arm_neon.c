...
void dspir_arm_neon_execute_f32(const dspir_transform *t,
    ...
    for (size_t stage = 0; stage < bits; stage++) {
        ...
        for (size_t group = 0; group < n / (2 * stride); group++) {
            ...
            /* Process 4 butterflies at a time with NEON */
            size_t i = 0;
            for (; i + 3 < stride; i += 4) {
                size_t idx0 = base + i;
                ...
                /* Butterfly */
                neon_bfly2(&a_re, &a_im, &b_re, &b_im, w_re, w_im);
                
                /* Store back */
                vst1q_f32(&work[2*idx0], a_re);
                vst1q_f32(&work[2*idx0 + 4], a_im);
                vst1q_f32(&work[2*idx1], b_re);
                vst1q_f32(&work[2*idx1 + 4], b_im);
            }
